...
| **Ready next** | **S0-5 historical coverage audit — GO now** (raw history and measured `canonical_freq=1min` are available), in parallel with the S0-2 historian follow-up and S0-4 certificate collection |
...
| **S0-5** | Historical coverage audit → `docs/phase0_data_audit.md` | ⏭️ **Ready to start — GO** | Raw history exists, but coverage, gaps, maintenance/outage/curtailment intervals, and empirical monthly `k_c`/regime distributions have not been derived. Never substitute an assumed Indonesian monsoon calendar. | Data Eng |
...
**M0 remains open: 2/7 tasks are fully accepted. S0-1 and S0-3 are complete; S0-2 remains 🟡 on historian confirmation.** **S0-3 decision: COMPLETE.** **S0-5 decision: GO now** — raw history and the measured `canonical_freq` exist; carry the unconfirmed historian timestamp-semantics caveat inside the audit instead of waiting on it. **S0-4 decision: consolidation delivered; 🟡 pending serial/calibration certificates and mapping/geometry confirmations.** **NO-GO for Phase 1 and all modelling.**
