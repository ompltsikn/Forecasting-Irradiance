...
| Version | 2.1 (Sprint 0 audit; S0-7 GO) |
...
> **Operational snapshot:** 2026-07-19 14:22 UTC. Status legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. A task receives ✅ only when its stated deliverable and acceptance evidence both exist.
...
| **S0-7** | ⏭️ **GO / ready to start (0/1)** | Manual SCADA export is known as the current offline path, but no written OT-security decision or dated commitment is recorded. | Hold/escalate the OT-security conversation now and record the achievable production data path, cadence, latency, named owner, and decision date. |
