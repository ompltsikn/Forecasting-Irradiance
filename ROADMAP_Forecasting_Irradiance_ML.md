...
> **Snapshot:** 2026-07-17 08:26 UTC. Legend: ✅ complete · 🟡 in progress · ⏭️ ready to start · ⬜ not started. Green means the deliverable **and** its acceptance evidence exist; it does not mean “the code was written.”

| ID | Status | Verified progress | Remaining gate / next action |
|---|---|---|---|
| **S0-1** | 🟡 **Implementation/activation complete; observation 1/2** | IFS + AIFS archiver, schema, hourly workflow, and separate issue/valid/retrieval timestamps are implemented; **166 local tests pass**. Manual smoke/full/catch-up succeeded. The first qualifying post-cutover [`schedule` run 29560032016](https://github.com/ompltsikn/Forecasting-Irradiance/actions/runs/29560032016) succeeded at 06:26 UTC and committed/read-back-validated AIFS issue `2026-07-17T00:00:00Z`; its IFS job skipped already-committed cycles. Shared Drive now has **11 IFS + 12 AIFS** paired Parquet/manifests at `gdrive:nwp`. | Observation is **1/2**. Verify a further qualifying scheduled event committing the next successive six-hour issue cycle. No second post-cutover scheduled run existed through 08:26 UTC; older/manual runs do not count. |
