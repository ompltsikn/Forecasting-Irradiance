...
| **S0-5** | ⏭️ **Ready to start — GO** | Historical raw files are available (145 local COV ZIPs; 2,028-workbook Drive inventory with the audited 156-file instantaneous closure subset), and S0-2's measured `canonical_freq=1min` gives the audit grid. S0-4 forwarded known outage leads: WS-3 GHI down from March 2026; RSI WS3.1/WS4.3 removed/failed. | Audit coverage/gaps/outages/curtailment and derive the site's monthly `k_c`, variability, and regime distributions; publish `docs/phase0_data_audit.md`. Carry the unconfirmed historian timestamp-semantics caveat explicitly. |
